    ...
    public class FunctionInvocationMiddleware
    {
        ...
        private async Task<bool> AuthenticateAndAuthorizeAsync(HttpContext context, FunctionDescriptor descriptor)
        {
            ...
            bool shouldAuthorize = (!descriptor.Metadata.IsProxy && httpTrigger?.AuthLevel != AuthorizationLevel.Anonymous) ||
                context.Request.Headers.ContainsKey(ScriptConstants.EasyAuthIdentityHeader) ||
                context.Request.Headers.ContainsKey(AuthenticationLevelHandler.FunctionsKeyHeaderName) ||
                context.Request.Query.ContainsKey(AuthenticationLevelHandler.FunctionsKeyQueryParamName);
